"""FeatureFilterSummaryOperator — 变量筛选汇总。"""

import pandas as pd

from .._base import ReportOperator, ReportSectionResult, SubSection
from .._context import ReportContext


class FeatureFilterSummaryOperator(ReportOperator):
    """变量筛选汇总算子。

    展示流水线每步筛选指标的阈值、筛选数量和剩余数量。
    """

    @property
    def name(self) -> str:
        return "feature_filter_summary"

    @property
    def title(self) -> str:
        return "变量筛选汇总"

    def compute(self, context: ReportContext) -> ReportSectionResult:
        attrs = context.pipeline_attrs
        if attrs is None:
            return ReportSectionResult(
                sheet_name=self.title,
                sub_sections=[SubSection(title=self.title, data=pd.DataFrame(), note="流水线属性未提取")],
            )

        # 计算初始特征数（原始输入列数）
        initial_n = None
        if attrs.feature_names_in_ is not None:
            # 入模特征数 = 最终保留
            final_n = len(attrs.feature_names_in_)
        else:
            final_n = 0

        # 从 X_train 推算初始数
        if context.X_train is not None:
            initial_n = context.X_train.shape[1]
        elif attrs.n_features_in_ is not None:
            initial_n = attrs.n_features_in_

        # 构建筛选步骤表
        rows = []
        remaining = initial_n

        # 缺失率筛选
        threshold = attrs.missing_threshold_
        drop_count = len(attrs.drop_columns_) if attrs.drop_columns_ else 0
        rows.append({
            "筛选指标": "缺失率",
            "阈值": f"＜{threshold}" if threshold else "未配置",
            "筛选变量数量": drop_count,
            "剩余变量数量": remaining - drop_count if remaining else "未知",
        })
        if remaining:
            remaining -= drop_count

        # 单一值筛选（含在 FeatureCleaner drop_columns_ 中）
        rows.append({
            "筛选指标": "单一值",
            "阈值": "＝1",
            "筛选变量数量": 0,  # 合并在缺失率筛选中，单独难以拆分
            "剩余变量数量": remaining if remaining else "未知",
        })

        # IV 筛选
        if attrs.iv_values_ is not None and attrs.iv_threshold_ is not None:
            iv_filter_count = int(((attrs.iv_values_ < attrs.iv_threshold_) | (attrs.iv_values_ > (attrs.iv_max_iv_ or 0.5))).sum())
            rows.append({
                "筛选指标": "IV",
                "阈值": f"＞{attrs.iv_threshold_} & ＜{attrs.iv_max_iv_ or 0.5}",
                "筛选变量数量": iv_filter_count,
                "剩余变量数量": remaining - iv_filter_count if remaining else "未知",
            })
            if remaining:
                remaining -= iv_filter_count
        else:
            rows.append({
                "筛选指标": "IV",
                "阈值": "未配置",
                "筛选变量数量": "未知",
                "剩余变量数量": remaining if remaining else "未知",
            })

        # PSI 稳定性筛选
        if attrs.psi_values_ is not None and attrs.psi_threshold_ is not None:
            psi_filter_count = int((attrs.psi_values_ > attrs.psi_threshold_).sum())
            rows.append({
                "筛选指标": "PSI",
                "阈值": f"＜{attrs.psi_threshold_}",
                "筛选变量数量": psi_filter_count,
                "剩余变量数量": remaining - psi_filter_count if remaining else "未知",
            })
            if remaining:
                remaining -= psi_filter_count
        else:
            rows.append({
                "筛选指标": "PSI",
                "阈值": "未配置",
                "筛选变量数量": "未知",
                "剩余变量数量": remaining if remaining else "未知",
            })

        # 相关性筛选
        if attrs.drop_features_ is not None and attrs.corr_threshold_ is not None:
            corr_filter_count = len(attrs.drop_features_)
            rows.append({
                "筛选指标": "相关性",
                "阈值": f"＜{attrs.corr_threshold_}",
                "筛选变量数量": corr_filter_count,
                "剩余变量数量": remaining - corr_filter_count if remaining else "未知",
            })
            if remaining:
                remaining -= corr_filter_count
        else:
            rows.append({
                "筛选指标": "相关性",
                "阈值": "未配置",
                "筛选变量数量": "未知",
                "剩余变量数量": remaining if remaining else "未知",
            })

        # 重要性筛选
        rows.append({
            "筛选指标": "重要性",
            "阈值": "≠0",
            "筛选变量数量": 0,
            "剩余变量数量": final_n if final_n else remaining,
        })

        df = pd.DataFrame(rows)
        return ReportSectionResult(
            sheet_name=self.title,
            sub_sections=[SubSection(title="3.变量筛选", data=df)],
        )
