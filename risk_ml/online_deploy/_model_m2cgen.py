...
并修正精度：输入一次性 float32 量化 + 阈值烘焙为 float32 舍入字面量，
纯 float64 比较精确等价于 float32 语义（实测 max diff ~1e-7，零翻转）。

生成的代码为纯标准库 Python（仅 struct），零 numpy 依赖，可内嵌字符串。
"""
...
def transpile(booster, base_prob, feature_names):
    ...
    lines = [
        ...
        "def score(input):",
        f"    q = [_f32(v) for v in input]   # 一次性 float32 量化({len(feature_names)} 特征)",
        "",
    ]
...
class M2CgenBackend:
    ...
    def score(self, X):
        ...
        if self._fn is None:
            ...
            self._fn = ns["score"]
        f = self._fn
        return np.array([f(list(row)) for row in np.asarray(X, dtype=np.float64)])
